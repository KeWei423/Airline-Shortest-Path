...
void airlinePath::loadMap()
{
    ...
    for(size_t i = 0; i < portElements.size(); i++)
    {
        ...
        table.insert(pair<int, vertex*>(portPtr->airportID, portPtr));
    }
}
...
void airlinePath::clear()
{
    ...
    beginID = endID = distance = totalCost = err = NULL;
    path.clear();
    for(map<int, vertex*>::iterator it = table.begin(); it != table.end(); it++)
    {
...
void airlinePath::trimAndUpper(string &line)
{
...
{
    string toPortMsg[] = { "- Leave ", "- Board ", "- Enter " };
    string toLineMsg[] = { " on ", " and board ", " onto " };
    string distMsg[] = { " and fly ", " for ", " after flying " };
    string multiLine = " and choose between ";
    string destPort = "- Finally, you will arrive at ";
    vertex *airport, *next;
    bool costError = 0;

    // Display cities from and to
    cout<<"From "<<table.at(beginID)->city<<" to "<<table.at(endID)->city<<endl;
    ...
    for(int i = 0; i < path.size()-1; i++)
    {
        ...
        cout<<toPortMsg[i%3]<<"("<<airport->iata<<") "<<airport->name;
        ...
        for(int i = 0; i < airport->routes.size(); i++)
        {
            ...
            if(rPtr->destinationID == next->airportID)
            {
                ...
                if(rPtr->airlines.size() >= 2)
                {
                    ...
                    {
                        cout<<"("<<aPtr->iata<<") "<<aPtr->name<<", ";
                    }
                    aPtr = rPtr->airlines[rPtr->airlines.size()-1];
                    cout<<"and ""("<<aPtr->iata<<") "<<aPtr->name;
                }
                ...
                {
                    airline* aPtr = rPtr->airlines[0];
                    cout<<toLineMsg[i%3]<<"("<<aPtr->iata<<") "<<aPtr->name;
                }

                // Display distance
                cout<<distMsg[i%3]<<rPtr->distance * (float)0.0006<<" miles. ";

                // Display cost
                if(airport->fare > 0)
                    cout<<"($"<<airport->fare<<")"<<endl;
                else
                    costError = 1;
    ...
    // Final destination
    cout<<destPort<<"("<<next->iata<<") "<<next->name<<endl<<endl;

    // Total distance and cost
    cout<<"There will be a total of "<<path.size()<<" airports to stop at."<<endl;
    cout<<"You will have traveled a total of: "<<distance * (float)0.0006<<" miles."<<endl;
    if(!costError)
        cout<<"The total cost for this trip will be $"<<totalCost<<endl<<endl;
}

void airlinePath::displayError()
{
    cout<<errMsg[err]<<endl<<endl;
}
