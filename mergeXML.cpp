...
{
    // Files and XML
    QString filePath = "/home/allen/School/CS8/finaldata/Resources/XML/";
    ...
    xmlInfo routeHeaders[4];
    xmlInfo airlineHeaders[2];
...
void loadXMLHeaders(xmlInfo port[8], xmlInfo rout[3], xmlInfo line[2])
{
    port[0].opening = "<id>";
    port[0].closing = "</id>";
    port[1].opening = "<name>";
    ...
    rout[1].opening = "<destinationAirportID>";
    rout[1].closing = "</destinationAirportID>";
    rout[2].opening = "<airlineID>";
    rout[2].closing = "</airlineID>";
    rout[3].opening = "<distance>";
    rout[3].closing = "</distance>";
...
void loadRoutes(map<int,airport*> &airportMap, QDomDocument &routes)
{
    ...
    for(size_t i = 0; i < elements.size(); ++i)
    {
        ...
        else if(airportMap.at(sourceID)->routeMap.find(destID) != airportMap.at(sourceID)->routeMap.end())
        {
            airportMap.at(sourceID)->routeMap.at(destID)->airlineMap.insert(pair<int, airline*>(airlineID, NULL));
        }
...
void writeRouteXML(ostream& out, airport* currPort, xmlInfo rout[], xmlInfo line[])
{
    ...
    for(map<int, route*>::iterator rit = currPort->routeMap.begin(); rit != currPort->routeMap.end(); rit++)
    {
        ...
        out<<"\t\t\t\t"<<rout[3].opening<<currRoute->distance<<rout[3].closing<<endl;
